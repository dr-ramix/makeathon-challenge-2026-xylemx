"""VGG-based UNet wrapper using timm feature extractor."""
...
class VGGUNet(nn.Module):
    """UNet decoder on top of a VGG encoder from timm.

    Requires `timm` to be installed.
    """
    ...
    def __init__(self, in_channels: int = 3, pretrained: bool = False, dropout: float = 0.0) -> None:
        ...
        try:
            ...
            raise RuntimeError("timm is required for VGGUNet (pip install timm)") from exc

        encoder = timm.create_model("vgg16", pretrained=pretrained, features_only=True, in_chans=in_channels)
