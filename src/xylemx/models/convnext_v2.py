"""ConvNeXtV2-based UNet wrapper using timm feature extractor."""
...
class ConvNeXtV2UNet(nn.Module):
    """UNet decoder on top of a ConvNeXtV2 encoder from timm."""
    ...
    def __init__(self, in_channels: int = 3, pretrained: bool = False, dropout: float = 0.0) -> None:
        ...
        try:
            ...
            raise RuntimeError("timm is required for ConvNeXtV2UNet (pip install timm)") from exc

        encoder = timm.create_model("convnextv2_tiny", pretrained=pretrained, features_only=True, in_chans=in_channels)
