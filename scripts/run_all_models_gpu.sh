...
mkdir -p output/train_runs output/predictions
...
RUN_PREFIX="${RUN_PREFIX:-all_models_gpu_script}"
EPOCHS="${EPOCHS:-20}"
...
echo "Running GPU multi-model training on host: $(hostname)"
...
for MODEL in "${MODELS[@]}"; do
  ...
  echo
  echo "============================================================"
  echo "Starting GPU training for model: ${MODEL}"
...
echo "All GPU model runs finished at: $(date -u)"
