...
RUN_PREFIX="${RUN_PREFIX:-all_models_local}"
EPOCHS="${EPOCHS:-20}"
...
echo "Running local multi-model training on host: $(hostname)"
...
for MODEL in "${MODELS[@]}"; do
  ...
  echo
  echo "============================================================"
  echo "Starting local training for model: ${MODEL}"
...
echo "All local model runs finished at: $(date -u)"
